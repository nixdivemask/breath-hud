...
type Profile = "stable" | "slowing" | "fast" | "crash" | "fading";
type RGB = [number, number, number];
type Sample = { t: number; bpm: number; amp: number };
...
function loadCount(): number {
  ...
  try {
    const n = Number(JSON.parse(localStorage.getItem(KEY) || "{}").n);
...
let people = makePeople(loadCount());
countEl.value = String(people.length);
...
function profileTarget(p: Person, t: number, vary: boolean): number {
...
function profileAmp(p: Person, t: number, vary: boolean): number {
...
function step(now: number) {
  ...
  lastTs = now;
  const t = now / 1000;
  const vary = varyEl.checked;
  for (const p of people) {
    p.target = profileTarget(p, t, vary);
    p.bpm += (p.target - p.bpm) * Math.min(1, dt * 0.7);
    p.amp = profileAmp(p, t, vary);
...
function frame(now: number) {
  ...
  const { cols, rows } = grid(people.length);
  const top = h * 0.09;
...
countEl.addEventListener("change", () => {
  const n = Math.max(2, Math.min(24, Math.round(Number(countEl.value) || 12)));
